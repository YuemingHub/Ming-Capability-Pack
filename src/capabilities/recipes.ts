...
export const RECIPES: Recipe[] = [
  ...
  {
    id: 'writing-document',
    name: '文字作品（简历/报告/文章/邮件/文案）',
    description: '把要求/素材写成一篇能直接拿去用的文字作品（简历、报告、文章、邮件、策划案等），结构完整、内容真实、无占位无编造',
    triggers: [
      '写份', '写篇', '帮我写份', '帮我写篇', '帮我写个',
      '起草', '润色', '改写',
      '简历', '求职信', 'cover letter', '自我介绍',
      '年终总结', '工作总结', '月度总结', '季度总结',
      '邮件', '回邮件', '写邮件', '回复邮件', '回信',
      '文案', '策划案', '写文章', 'essay', 'resume', 'cv',
    ],
    guidance: [
      '先确认用途与对象（写给谁、什么场景、什么风格）再动笔；素材在文件里就先读取，不编造用户没提供的事实',
      '结构匹配用途：简历=个人信息+经历+技能；报告=结论先行+要点支撑；文章=开头抓住+层次清晰；邮件=目的+要点+结尾',
      '内容真实红线：用户没提供的日期/数字/头衔不编造，缺的信息明确标注「此处需你补充」，绝不填假信息',
      '不用模板腔与大话套话：数字与事实具体，读的人知道怎么用',
      '产出 .md 文件，完成后给出文件路径与结构说明',
    ],
    capabilities: [
      { kind: 'tool', id: 'fs_*', purpose: '读取素材与产出文字文件', trust: 'official' },
    ],
    delegate: { provider: 'spawn' },
    verification: [
      { kind: 'file_exists', pattern: '*.md', note: '应产出文字文件' },
      { kind: 'content_absent', pattern: '*.md', mustNotContain: 'TODO', note: '绝无未填占位' },
      { kind: 'content_absent', pattern: '*.md', mustNotContain: '由AI生成', note: '绝无工具水印' },
    ],
    questions: [
      {
        key: 'docType',
        question: '这次写的是什么？（文章 / 简历 / 报告 / 邮件 / 策划案）',
        default: '文章',
        options: ['文章', '简历', '报告', '邮件', '策划案'],
        translate: '用户说「找工作/面试/投」→ 简历；「汇报/总结/复盘」→ 报告；「发给别人/邀请/通知」→ 邮件；「方案/活动计划」→ 策划案；默认 → 文章',
      },
      {
        key: 'style',
        question: '想要正式一点还是亲切一点？',
        default: '正式',
        options: ['正式', '亲切', '活泼'],
        translate: '用户说「认真/商务/汇报用」→ 正式；「轻松/口语/日常」→ 亲切；「有趣/生动」→ 活泼',
      },
      {
        key: 'length',
        question: '大概要多长？',
        default: '适中（一页左右）',
        options: ['短（几行到半页）', '适中（一页左右）', '长（几页）'],
        translate: '用户说「简短/几句话/简单」→ 短；「详细/全面/深入」→ 长；默认 → 适中',
      },
    ],
    qualityBar: {
      bar: '这一轮就交付「能直接拿去用的文字」——结构完整、内容真实、无占位无编造',
      checks: [
        '结构匹配用途：简历=个人信息+经历+技能；报告=结论先行+要点支撑；文章=开头抓住+结构清晰；邮件=目的+要点+结尾',
        '内容真实：用户素材里的信息用全；没有的事实不编造——缺的明确标注「此处需你补充」',
        '可落地：不用模板腔/大话套话，数字与事实具体，读的人知道怎么用',
        '留修改入口：交付时给出「哪几处可能想改」的引导',
      ],
      selfCheck: [
  ...
  },
  {
    ...
    description: '把一段长内容提炼成结构化要点：核心结论 + 分点要点 + 行动项，关键信息不丢、不编造，可直接转发',
    triggers: [
      '总结', '提炼', '摘要', '太长', '浓缩', '要点',
      '会议纪要', '聊天记录', '谈话记录', '纪要',
      '太长不看', 'summarize', 'summary',
    ],
    guidance: [
      '素材获取：用户提到「文章/文档/记录/聊天」时，先用文件工具自己定位并读取素材（.md/.txt/.docx 等常见格式，在用户工作区/常见文档位置找）；读不了就如实说明缺什么解析能力，确实找不到时最多问一次大概位置',
      '提炼红线：只提炼不创作——原文没有的结论不加，数字/人名/时间只来自原文；原文含糊处标注「原文不清，需确认」',
      '结构：核心结论 → 分点要点 → 行动项（如有）；长度约为原文 10%，太长=没提炼，太短=丢了信息',
      '产出 .md 文件，完成后给出文件路径与结构说明',
    ],
    capabilities: [
      { kind: 'tool', id: 'fs_*', purpose: '读取素材与产出提炼文件', trust: 'official' },
    ],
    delegate: { provider: 'spawn' },
    verification: [
      { kind: 'file_exists', pattern: '*.md', note: '应产出提炼文件' },
      { kind: 'content_absent', pattern: '*.md', mustNotContain: 'TODO', note: '绝无未填占位' },
    ],
    questions: [
      {
        ...
        question: '总结到什么程度？',
        default: '分点要点（每点一句话）',
        options: ['只要核心结论（一段话）', '分点要点（每点一句话）', '要点 + 行动项'],
        translate: '用户说「就说结论/大概讲什么」→ 核心结论；「要点/重点/归纳」→ 分点要点；「看完我要做什么/下一步」→ 要点+行动项',
      },
      {
        key: 'source',
        question: '内容从哪里来？（有文件就告诉我大概在哪个文件夹，没有就发我文字）',
        default: '先用文件工具读工作区里的内容',
        options: ['先用文件工具读工作区里的内容', '我直接把内容发给你'],
        translate: '用户给了文件/位置 → 自己定位读取；用户直接贴文字 → 用贴的内容提炼',
      },
    ],
    qualityBar: {
  ...
  },
  {
    ...
    description: '把一份混乱的表格整理成规整、汇总正确、可溯源的结果：先确认理解、每步有依据、改动有记录',
    triggers: [
      '表格', 'excel', 'xlsx', 'csv', '汇总', '数据整理', '整理数据',
      '清洗', '核对', '统计', '数据对不上', '批量整理', '格式统一',
    ],
    guidance: [
      '动手前先复述「这是什么表、要做什么」，确认没会错意再动（在交付说明里写明复述与确认结果）',
      '素材获取：先用文件工具自己定位并读取表格文件（.xlsx/.csv/.tsv 等）；读不了（如缺 excel_read）就如实说明，并用 ming_store_search 找表格解析插件；确实找不到时最多问一次大概位置',
      '产出 .csv 结果文件（UTF-8 带 BOM，Excel 打开不乱码）+ .md 处理说明（改了什么、每步怎么算的、原始 vs 结果对照）',
      '清洗不静默：删除/修改/合并都有记录，可恢复',
    ],
    capabilities: [
      { kind: 'tool', id: 'fs_*', purpose: '读写表格与产出文件', trust: 'official' },
      {
        kind: 'tool',
        id: 'excel_read',
        source: 'dsh-univer-office',
        purpose: '读取/编辑 Excel 表格数据（dsh-univer-office：表格/文档/演示/数据库，支持实时预览）',
        trust: 'community',
        optional: true,
      },
    ],
    delegate: { provider: 'spawn' },
    verification: [
      { kind: 'file_exists', pattern: '*.csv', note: '应产出规整后的结果文件（.csv）' },
      { kind: 'file_exists', pattern: '*.md', note: '应产出处理说明（改了什么、怎么算的）' },
    ],
    questions: [
      {
        key: 'task',
        question: '拿到数据后主要想做什么？',
        default: '汇总统计',
        options: ['汇总统计', '清洗整理', '核对查错', '格式统一'],
        translate: '用户说「统计/合计/平均/排名」→ 汇总统计；「太乱/重复/空行/清理」→ 清洗整理；「对不上/错了/查一下」→ 核对查错；「格式不统一/单位/日期」→ 格式统一',
      },
      {
        key: 'source',
        question: '表格文件在哪？（告诉我大概文件夹即可，我自己找）',
        default: '先用文件工具在工作区里找',
        options: ['先用文件工具在工作区里找', '我发文件路径给你'],
        translate: '用户给了路径/文件夹 → 自己定位读取；只有一句话描述 → 在工作区常见位置搜索',
      },
    ],
    qualityBar: {
