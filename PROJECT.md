...
3. **能力装配（轻装配）**：curated 官方能力自动装（用户无感）→ 社区能力一句确认 → 市场兜底（Marketplace 优先 + 1024Store 兜底），只给跑得通的命令（过滤 #path 不可装候选）；安装状态机 verified/pending/absent（未确认写入绝不报已装）；重型装配交给生态插件 dsh-plugin-autoevo。
